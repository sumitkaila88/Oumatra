...
- Stacked lockup: 28-unit tagline below the wordmark in an `860 × 480` field.
...
All files have explicit viewBoxes, contain no raster images, and avoid external references or embedded font files. The deterministic generator at `scripts/generate-brand-assets.mjs` is the master source for geometry, lighting and variant consistency.
